...
if __name__ == "__main__":
  ...
  for i in range(3, 4):
    ...
    model_list = main(influencers = influencer, cdim = i, method="new")
    ...
    model_true_errs = np.array([model_experiment.true_err.dict[key] for key in matrix_experiment.err_matrix.dict])
    model_pred_errs = []
    ...
    model_pred_errs = np.array(model_pred_errs).flatten()
    ...
    matrix_abs_diffs = abs(matrix_pred_errs - matrix_true_errs)
    model_abs_diffs = abs(model_pred_errs - model_true_errs)
    plt.scatter(p_values, matrix_abs_diffs, label="Matrix")
    plt.scatter(p_values, model_abs_diffs, label="Model")
    # plt.scatter(list(matrix_experiment.p_dot.values()), abs((np.ones(len(matrix_true_errs)) - matrix_pred_errs) - matrix_true_errs))
